...
For serveral years, Bokeh used a constraint solver to compute the layout
...
available were eitehr broken, or not implemented fully. I am happy to report
that all the sizing modes such as `"stretch_both"` and `"scale_wifth"` are
now implimented fully:
...
updates that could have undetected and unintentional changes.
...
* Selenium testing to increase reliability and stability
