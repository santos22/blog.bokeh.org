...
* An old is for a ``FileInput`` widget is finished.
...
* A few more ``DataTable`` bugs were fixed.
...
We will publish a blog post soon outlining expected Bokeh 2.0 changes.
...
As a reminder, the Bokeh has launched two new sites recently:
...
page on LinkedIn. This was mainly to offer additional log-in options on
the Discourse site. However, it's also useful for anyone who has contributed to
Bokeh to be able to list on their profile if they desire.
...
We will make a wrap-up blog post about the fundraising experience, in July. Now, on to new features!
...
improved and refine the initial version.
...
you specify. It can evenly automatically aggregsate those groups if you
