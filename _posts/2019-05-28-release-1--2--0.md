...
<link
    href="https://cdn.pydata.org/bokeh/dev/bokeh-1.2.0rc1.min.css"
    rel="stylesheet" type="text/css">

<script src="https://cdn.pydata.org/bokeh/dev/bokeh-1.2.0rc1.min.js"></script>
...
* Styling ptions for fill patterns / hatching [`#6135`](https://github.com/bokeh/bokeh/issues/6135)
...
*The Google Groups mailing list has been retired*. In it's place, we have stood
up a Discourse instance to host discussions. All of the old content from the
...
text, rich media, as well as search and traffic statistics that will help us
better improve the site and our docs.
...
Even well before [`#6135`](https://github.com/bokeh/bokeh/issues/6135), wich is already
a few years old at this point, I had wanted to be able to support fill patterns (or
"hatching") for things like bars are patches. I am please to report that Bokeh now
supports a wide variety of [Hatch Properties](https://bokeh.pydata.org/en/1.2.0/docs/user_guide/styling.html#hatch-properties) for specifying fill patterns:
...
Besides theses built-in hatch patterns, it's also possible set fill patterns
from any image, as long as the image is available at a URL (base64 data URLs will
work too):
...
These are nice enough on their own, but they are even more useful to
use for creaing stacked area charts:
...
For a full example, check out the update [*stacked_area.py*](https://github.com/bokeh/bokeh/blob/1.2.0/examples/plotting/file/stacked_area.py)
...
```python
slider.callback_polich = "mouseup"
...
Also note that the old-style `callback` property will be removbed in
...
up. The grant program is specifically targetting Open Source projects that are
